  ...
  switch (current.type) {
    case "scenario":
      ...
      return {
        ...current,
        executor:
          current.executor && replace(current.executor, target, replacement),
...
function findBlockById(current: Block | null, id: string): Block | null {
  ...
  switch (current.type) {
    case "scenario":
      return (
        findBlockById(current.executor, id) ?? findBlockById(current.step, id)
      );
    ...
    case "check":
      return (
        findBlockById(current.next, id) ?? findBlockById(current.target, id)
      );
...
function isDescendantOf(current: Block | null, target: Block): boolean {
  ...
  switch (current.type) {
    case "scenario":
      return (
        isDescendantOf(current.executor, target) ||
        isDescendantOf(current.step, target)
      );

    case "group":
      return (
        isDescendantOf(current.step, target) ||
        isDescendantOf(current.next, target)
      );

    case "check":
      return (
        isDescendantOf(current.next, target) ||
        isDescendantOf(current.target, target)
      );
